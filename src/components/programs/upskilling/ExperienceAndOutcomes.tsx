...
export function ExperienceAndOutcomes() {
  const experiences = [
    { icon: Video, title: "Live Interactive Classes", desc: "Two 90-minute live sessions per week focusing purely on building and debugging." },
    { icon: Trophy, title: "Weekly Assignments", desc: "Build a specific micro-workflow each week to lock in your muscle memory." },
    { icon: Users, title: "Mentorship & Office Hours", desc: "Stuck on an API error? Jump into office hours and screen-share with instructors." },
    { icon: MessagesSquare, title: "Community Support", desc: "A private Discord filled with fellow builders, resources, and job opportunities." }
  ];
                  ...
                  <React.Fragment key={idx}>
                    <motion.div
                      ...
                      className={`flex flex-col items-center text-center p-4 rounded-xl ${
                        isLast ? "bg-white text-brand shadow-xl scale-110" : "bg-black/10 text-white"
                      }`}
                    >
                      {isLast && <Briefcase className="w-6 h-6 mb-2 text-brand" />}
                      <span className={`text-sm md:text-xs lg:text-sm font-semibold uppercase tracking-wider ${
